...
export default function GraphView({ businessUnit }) {
  ...
  const [layingOut, setLayingOut] = useState(false); // FA2 worker still settling
  ...
  useEffect(() => {
    ...
    subgraph({ businessUnit, domains, limit: 6000 }).then((data) => {
      ...
      data.nodes.forEach((n, i) => {
        ...
        const a = (i / count) * 2 * Math.PI; // circular seed — stable start for the worker layout
        g.addNode(n.id, {
      ...
      // The fetch can resolve mid-layout-transition (tab/mode switches), when the
      // container exists but hasn't been given a width yet — Sigma throws on that.
      // Wait a frame until it has real dimensions.
      const mount = () => {
        if (killed) return;
        const el = containerRef.current;
        if (!el) return;
        ...
        // ForceAtlas2 runs in a web worker — the old synchronous assign() locked the
        // main thread for seconds at 6k nodes. The graph stays interactive and visibly
        // settles; we stop the simulation once it has had time to converge.
        const layout = new FA2Layout(g, { settings: forceAtlas2.inferSettings(g) });
        layoutRef.current = layout;
        setLayingOut(true);
        layout.start();
        timer = setTimeout(() => {
          if (layoutRef.current === layout) { layout.stop(); setLayingOut(false); }
        }, Math.min(8000, Math.max(2500, g.order)));
      };
      mount();
  ...
  return (
    <div className="h-full flex flex-col md:flex-row">
      ...
      <aside className={`w-56 shrink-0 border-r border-white/10 p-3 overflow-auto text-sm bg-[#0b0f14] md:static md:block
        ...
        <h3 className="text-white/40 text-xs uppercase tracking-wide mt-4 mb-2">Legend</h3>
        <div className="space-y-1">
          {presentTypes.map((t) => (
      ...
      <div className="relative flex-1 min-w-0 min-h-0 max-md:h-[60vh]">
        ...
        <div className="absolute top-3 left-3 text-xs text-white/50 bg-black/40 rounded px-2 py-1 max-md:hidden">
          {loading ? "loading graph…" : `${stats.nodes} nodes · ${stats.edges} edges${layingOut ? " · settling layout…" : ""}`}
        </div>
      </div>
